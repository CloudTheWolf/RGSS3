...
# (en) User Guide           : http://pastebin.com/raw.php?i=EgnWt9ur
 #################
# Modded by CloudTheWolf to work with Basic Window Resizer v1.1 by: V.M of D.T
#################
$imported ||= {}
$imported[:Zeus_Fullscreen] = __FILE__
$imported["Cloud-WindowSetting"]
class << Graphics
...
public
  def load_fullscreen_settings
    ...
    @fitscreen       = get_option.call('Fitscreen'     , '0') == '1'
  ...
  def save_fullscreen_settings
    ...
    set_option.call('Fitscreen'     , @fitscreen ? '1' : '0')   
  ...
  def fullscreen_mode
    ...
    show_back
    hide_borders # Hide Window Border
    @fullscreen = true
    @fitscreen = false
    self.ratio += 0 # refresh window size
    Window_Resize.f
 ...
 def toggle_resize
    if @fitscreen == true
      #disable Fit Screen
      @fitscreen = false
      #Window_Resize.r(544, 416)      
      #windowed_mode #Disable Fullscreen as well as Fit Screen
      self.ratio += 0
      save_fullscreen_settings
    else
      #enable fit screen 
...
def mode_test
    if @fitscreen == true
      toggle_resize
      toggle_ratio
      #Win32API.new('user32.dll','MessageBoxA',['L', 'P', 'P', 'L'],'I').call(0,'Please disable Fit Screen Mode with F7 First.','I can not do that... soz!',0)
    else
      toggle_ratio 
    end
  end
  ...
  def update
    ...
    mode_test          if Input.trigger?(Input::F6)
    toggle_resize      if Input.trigger?(Input::F7)
